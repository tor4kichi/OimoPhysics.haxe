...
class BoxBoxCollisionDetector extends CollisionDetector
{
    ...
    override public function detectCollision(shape1 : Shape, shape2 : Shape, result : CollisionResult) : Void{
        ...
        if (shape1.id < shape2.id) {
            b1 = try cast(shape1, BoxShape) catch(e:Dynamic) null;
            b2 = try cast(shape2, BoxShape) catch(e:Dynamic) null;
        }
        else {
            b1 = try cast(shape2, BoxShape) catch(e:Dynamic) null;
            b2 = try cast(shape1, BoxShape) catch(e:Dynamic) null;
        }  // }    // 	trace("!?");    // if (shape1.id == shape2.id) {  
        ...
        if (numClipVertices == 0)             return;  // what's happened  ;
        numAddedClipVertices = 0;
